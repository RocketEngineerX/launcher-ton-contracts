...
export class Pool implements Contract {
    ...
    // these must be aligned with ops_and_errors.rc
    static errorCodes = {
        alreadyInitiated: 0xfff0,
        notAllowedBeforeInitiate: 0xfff1,
        notificationFromWrongAddress: 0xfff2,
        amountNotAvailable: 0xfff3,
    } as const;

    static estimatedGasPrices = {
        deploy: toNano('0.05'),
    } as const;
    ...
    async getEstimatedRequiredTonForJetton(provider: ContractProvider, jettonAmount: bigint) {
        ...
        try {
            ...
            if('exitCode' in error && error.exitCode == Pool.errorCodes.amountNotAvailable
             || 'message' in error && error.message.includes(`exit_code: ${Pool.errorCodes.amountNotAvailable}`)
            ) {
    ...
    async getPriceComponents(provider: ContractProvider) {
